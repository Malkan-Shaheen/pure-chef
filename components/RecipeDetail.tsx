...
export const RecipeDetail: React.FC<RecipeDetailProps> = ({ recipe, onBack, onStartKitchen, onFeedback }) => {
  ...
  const [servings, setServings] = useState(recipe.servings_count || 4);
  ...
  const handleSaveAsPDF = () => {
    ...
    doc.text(`Prep Time: ${recipe.prepTime}`, margin, y);
    y += 15;
    ...
    doc.text("Generated by PURE Chef AI", margin, 285);

    doc.save(`${recipe.title.replace(/\s+/g, '_')}_Recipe.pdf`);
  };

  const handleShare = async () => {
    const shareText = `Check out this recipe for ${recipe.title} from PURE Chef!\n\n${recipe.description}`;
    const isDataUrl = recipe.imageUrl?.startsWith('data:');
    
    // We don't share base64 strings in the URL field as they break navigators
    const shareUrl = isDataUrl ? window.location.href : (recipe.imageUrl || window.location.href);
  ...
  const currentNutrition = useMemo(() => {
    const total = recipe.nutrition_total || { calories: 0, protein_g: 0, carbs_g: 0, fat_g: 0 };
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-3xl overflow-hidden shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500 mb-12">
      ...
      <div className="p-8 md:p-12">
        ...
        {isGapRecipe && (
          <div className="mb-8 p-6 bg-amber-50 border-2 border-dashed border-amber-200 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6 no-print">
            ...
            <a 
              ...
              className="w-full md:w-auto flex items-center justify-center gap-3 bg-amber-400 hover:bg-amber-500 text-amber-900 font-bold py-4 px-8 rounded-2xl shadow-lg shadow-amber-200 transition-all hover:scale-[1.05]"
            >
        ...
        {/* AI Taste Memory Feedback Section */}
